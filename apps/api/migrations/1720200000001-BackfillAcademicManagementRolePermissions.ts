...
/**
 * Backfills the new `academic-management` module permissions onto system
 * roles for tenants provisioned before this module existed. Same pattern as
 * the two earlier Phase 1 backfills.
 */
export class BackfillAcademicManagementRolePermissions1720200000001 implements MigrationInterface {
  private readonly permissionsByRoleName: Record<string, Array<{ module: string; action: string }>> = {
    ...
    Teacher: [{ module: 'academic-management', action: 'view' }],
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const role of roles) {
      const existing = Array.isArray(role.permissions) ? role.permissions : [];
      const filtered = existing.filter((p: any) => p.module !== 'academic-management');
      await queryRunner.query(`UPDATE roles SET permissions = $1 WHERE id = $2`, [
