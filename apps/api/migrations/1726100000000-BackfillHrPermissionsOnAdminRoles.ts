...
export class BackfillHrPermissionsOnAdminRoles1726100000000 implements MigrationInterface {
  private readonly permissionsByRoleName: Record<string, Array<{ module: string; action: string }>> = {
    'Super Admin': ['view', 'create', 'edit', 'delete', 'approve'].map((action) => ({ module: 'hr-management', action })),
    'District/Trust Admin': ['view', 'create', 'edit', 'delete', 'approve'].map((action) => ({ module: 'hr-management', action })),
    'School Admin': ['view', 'create', 'edit', 'delete', 'approve'].map((action) => ({ module: 'hr-management', action })),
  };
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const role of roles) {
      const existing = Array.isArray(role.permissions) ? role.permissions : [];
      const filtered = existing.filter((p: any) => p.module !== 'hr-management');
